...
scripts/test_crash.py before being ported here):
  ...
  3. output_crash_json() nests its diagnostic fields under an "error" or "crash" object.
     The parser now reads that object instead of silently ignoring it.
...
def parse_output(output: str, returncode) -> Dict[str, Any]:
    ...
    # Read diagnostic fields from "crash" or "error" nested objects
    if result["status"] == "crash":
        err = result.get("crash") or result.get("error")
        if isinstance(err, dict):
            result["signal"] = str(err.get("signal") or err.get("details") or "")
            if "message" in err:
                result["message"] = str(err.get("message", ""))
            else:
                result["message"] = (
    ...
    # Distinguish a HANG (timeout, force-killed, no exit status) from a real crash
    ...
    # OS-level ground truth for terminating signals
    elif returncode is not None and returncode < 0 and not result["signal"]:
...
def insert_sdc(conn: sqlite3.Connection, run_id: int, result: Dict[str, Any]) -> None:
    ...
    conn.executemany(
        ...
        [
            (
                run_id,
                str(d.get("expected_v") or d.get("expected_value", "")),
                str(d.get("actual_v") or d.get("actual_value", "")),
                int(d["hamming_dist"] if "hamming_dist" in d else d.get("hamming_distance", 0)),
                int(d["index_value"] if "index_value" in d else d.get("index", 0))
            )
